...
> version installed, so `install` removes it. Say so in your own release notes
> when you drop one.
>
...
> Anything edited since, anything another tool wrote, a hand-written skill and
> anything `.`-prefixed are all left alone, and `--force` does not change that.
> `install <name>` sweeps nothing it was not told to.
...
is, check that `WithToolName` matches what the earlier build used.
